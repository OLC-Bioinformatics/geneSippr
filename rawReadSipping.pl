...
# Call configureBclToFastq.pl - look into preventing the compression of the .fastq files
unless(-e("$folderPath/Unaligned")) {
	system("configureBclToFastq.pl --input-dir $miSeqPath/$folder/Data/Intensities/BaseCalls/ --output-dir $folderPath/Unaligned --force --sample-sheet $folderPath/SampleSheet_modified.csv --mismatches 1 --no-eamss --fastq-cluster-count 0 --use-bases-mask $baseMask");
...
while (defined(my $file = readdir(DIR))) {
	...
	if ($file =~ m/Sample_/) {
		if (-d $path ."/". $file){
			...
			(my $filename = $files[0]) =~ s/_L001|.gz//g;
			running_time("Now extracting $filename");
			make_path("$path/$folder");
			make_path("$path/query");
			move($files[0],"$path/$folder/$files[0]");
			system("gzip -d $path/$folder/$files[0]");
			chdir("$path/$folder");
			if (-f $unzip){unlink "$path/$folder/$files[0]"};
			move("$path/$folder/$unzip","$path/$folder/$filename");
			copy("$path/$folder/$filename", "$path/query/$filename");
			if (-f $filename){rmdir("$path/$file") or die $!};
			chdir("$path");
		}
	}
}
